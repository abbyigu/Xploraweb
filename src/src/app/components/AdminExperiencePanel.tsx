...
export function AdminExperiencePanel() {
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      ...
      ({ error: dbError } = await supabase.from('xplora_experiences').insert(payload));
    }
    if (dbError) {
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            {form.category === 'xplorators' && (
              <div className="md:col-span-2">
                <label className="text-xs text-muted-foreground">Itinerary stops — one per line <span className="text-primary">(Xplora-tors only)</span></label>
