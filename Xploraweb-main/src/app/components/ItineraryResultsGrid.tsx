...
export function ItineraryResultsGrid({ itineraries, onRegenerate, onSaved }: Props) {
  if (itineraries.length === 1) {
    return (
      <div className="max-w-7xl mx-auto px-6 md:px-8 mt-8">
        <ItineraryResultCard itinerary={itineraries[0]} onRegenerate={onRegenerate} onSaved={onSaved} layout="full" />
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-6 md:px-8 mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      {itineraries.map((itinerary, i) => (
        <div key={i} className={i === 2 ? 'md:col-span-2 lg:col-span-1' : ''}>
          <ItineraryResultCard itinerary={itinerary} index={i} onRegenerate={onRegenerate} onSaved={onSaved} />
        </div>
      ))}
    </div>
  );
