...
import { Plus, Trash2, Eye, EyeOff, CheckCircle, AlertCircle, Star, MapPin, Mail, Globe, LayoutDashboard, Gift } from 'lucide-react';
...
export function DashboardBusinessPanel({ profile }: { profile: DashboardProfile }) {
  ...
  async function loadData() {
    ...
    const { data } = await supabase.from('business_perks').select('*').eq('business_id', user.id).order('created_at', { ascending: false });
  ...
  if (loading) return <div className="flex items-center justify-center py-24"><div className="w-6 h-6 border-2 border-primary border-t-transparent rounded-full animate-spin" /></div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-1 bg-muted rounded-xl p-1 w-fit">
        {([
          ...
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 ${tab === key ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
      ...
      {tab === 'offers' && <div className="space-y-8">
