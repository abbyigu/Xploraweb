...
export function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-border shadow-lg">
      <div className="max-w-md mx-auto flex items-center py-2 px-1">
        {navItems.map(({ path, icon: Icon, label, badge }) => (
          <Link
            key={path}
            to={path}
            className={`relative flex flex-1 min-w-11 flex-col items-center gap-0.5 px-0.5 py-2.5 min-h-[44px] rounded-lg transition-all ${
              isActive(path)
