...
export function SaveSpotButton({ spot, className }: { spot: Spot; className?: string }) {
  ...
  return (
    <button
      ...
      aria-pressed={saved}
      className={className ?? "absolute top-2 right-2 before:absolute before:-inset-x-0.5 before:-inset-y-1.5 before:content-[''] w-8 h-8 rounded-full bg-black/40 hover:bg-black/55 flex items-center justify-center transition-colors z-10"}
    >
